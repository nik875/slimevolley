...
class Network:
    def __init__(self, param_size=112):  # 105 weights + 7 biases
        ...
        self.weight = self.params[:-7].reshape(self.nOutput,
                                               self.nInput)  # 7x15 matrix = 105 parameters
        self.bias = self.params[-7:]  # 7 parameters
    ...
    def mutate(self, mutation_rate=0.1, mutation_std=0.1):
        ...
        self.weight = self.params[:-7].reshape(self.nOutput, self.nInput)
        self.bias = self.params[-7:]
    ...
    def crossover(self, other_network, crossover_rate=0.2):
        ...
        child.weight = child.params[:-7].reshape(child.nOutput, child.nInput)
        child.bias = child.params[-7:]
...
class GeneticAlgorithm:
    def __init__(self,
        ...
        self.population = [Network(param_size) for _ in range(population_size)]
...
if __name__ == "__main__":
    best_solution = train_genetic_algorithm(
        num_generations=2048,  # Increased generations
        pop_size=112 * 10,        # Double the population size
        workers=112,
        resume_from="population_20241114_180043",
        start_gen=512
    )
